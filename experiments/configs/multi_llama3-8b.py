...
def get_config(num_train_models):
    ...
    for i in range(len(subtasks)):
        config.tokenizer_paths.append(f"/fsx-project/yunyun/models/meta-llama_Meta-Llama-3-8B_{subtasks[i]}_tuned")
    ...
    for i in range(len(subtasks)):
        config.model_paths.append(f"/fsx-project/yunyun/models/meta-llama_Meta-Llama-3-8B_{subtasks[i]}_tuned")
    ...
    for i in range(len(subtasks)+1):
        config.conversation_templates.append("llama-3")

    config.devices = []
    if int(num_train_models)==3:
        for i in range(len(subtasks)+1):
            config.devices.append(f"cuda:{i+5}")
    elif int(num_train_models)==2:
        for i in range(len(subtasks)+1):

...
def get_config(num_train_models):
    ...
    for i in range(len(subtasks)):
        config.tokenizer_paths.append(f"/fsx-project/yunyun/models/mistralai_Mistral-7B-v0.3_{subtasks[i]}_tuned")
    ...
    for i in range(len(subtasks)):
        config.model_paths.append(f"/fsx-project/yunyun/models/mistralai_Mistral-7B-v0.3_{subtasks[i]}_tuned")
    ...
    for i in range(len(subtasks)+1):
        config.conversation_templates.append("mistral")

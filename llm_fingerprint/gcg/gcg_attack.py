...
from llm_fingerprint import AttackPrompt, MultiPromptAttack, PromptManager
from llm_fingerprint import get_embedding_matrix, get_embeddings
...
def token_gradients(model, input_ids, input_slice, target_slice, loss_slice):
    ...
    """
    # print('control slice shape: ', input_ids[input_slice].shape)
    # print('control slice shape: ', input_ids[input_slice].unsqueeze(1).shape)
    embed_weights = get_embedding_matrix(model)
    # print('embed weights: ', embed_weights.shape)
    one_hot = torch.zeros(
    ...
    full_embeds = torch.cat(
        ...
        dim=1)
    # print('full embed shape: ', full_embeds.shape)
    # print('input embeds shape: ', input_embeds.shape)
    logits = model(inputs_embeds=full_embeds).logits    
    # print('logits..... ', logits.shape)
    targets = input_ids[target_slice]
    # print(targets)
    # print('loss of logits..... ', logits[0,loss_slice,:].shape)
    loss = nn.CrossEntropyLoss()(logits[0,loss_slice,:], targets)
    
    loss.backward()
    # print('loss: ', loss.shape, ' | ', loss)
    # print('one hot gradient : ', one_hot.grad)
...
class GCGPromptManager(PromptManager):
    ...
    def sample_control(self, grad, batch_size, topk=256, temp=1, allow_non_ascii=False):
        ...
        # print(new_token_pos)
        # print('new token pos: ', len(new_token_pos), 'len of control toks: ', len(control_toks), 'batch size: ', batch_size)
    ...
    def step(self, 
        ...
        for j, worker in enumerate(self.workers):
            ...
            if grad.shape != new_grad.shape:
                ...
                grad += new_grad
            # print('worker {} grad.shape {}'.format(j, grad.shape))
        ...
        with torch.no_grad():
            ...
            # print(ids_list)
            # next_ids = ids_list[min_idx]
            
            next_control, cand_loss = control_cands[model_idx][batch_idx], loss[min_idx]
        
        del control_cands, loss ; gc.collect()
        # print('min ids : ', next_ids)
        print('Current length:', len(self.workers[0].tokenizer(next_control).input_ids[1:]))
        # print('next_control ids: ', self.workers[0].tokenizer(next_control, add_special_tokens=False).input_ids)
        # print('next control: ', next_control)

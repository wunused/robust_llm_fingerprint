...
class AttackPrompt(object):
    ...
    def _update_ids(self):
        ...
        encoding = self.tokenizer(prompt, add_special_tokens=False) ##add special tokens as false trying to fixed llama3 token bug......
        ...
        if self.conv_template.name == 'llama-2':
            ...
            if python_tokenizer:
                ...
                self.input_ids = torch.tensor(toks[:self._target_slice.stop], device='cpu')
                # print('input id in update id func.', self.input_ids)
    ...
    def generate(self, model, gen_config=None, find_target=False):
        ...
        input_ids = self.input_ids[:self._assistant_role_slice.stop].to(model.device).unsqueeze(0) #llama-3
        # input_ids = self.input_ids[:self._assistant_role_slice.stop].to(model.device).unsqueeze(0) #llama-2
        attn_masks = torch.ones_like(input_ids).to(model.device)
        # print('in generate function: ', input_ids)
        ...
        if find_target:
            ...
            perm_idx = [32000] * (len(sorted_indices)//2) - np.random.randint(1, max_perm_num, size=len(sorted_indices)//2)
            # print(perm_idx) 
            ...
            last_freq_indices = [sorted_indices[i, perm_idx[i]] for i in range(sorted_indices.shape[0]//2)]
            ...
            # generated_tokens = output.sequences[:, self._assistant_role_slice.stop:] #llama-2
            generated_tokens = output.sequences[:, self._assistant_role_slice.stop:] #llama-3
            output_ids = generated_tokens.squeeze(0)
    ...
    def eval_str(self):
        ...
        # return self.tokenizer.decode(self.input_ids[self._user_role_slice.start:self._assistant_role_slice.stop]).replace('<|begin_of_text|><|begin_of_text|>', '<|begin_of_text|>').replace('</s>','') #only for llama3
...
class MultiPromptAttack(object):
    ...
    def get_filtered_cands(self, worker_index, control_cand, filter_cand=True, curr_control=None):
        ...
        for i in range(control_cand.shape[0]):
            ...
            # print('decodeted str: ', decoded_str)
            # p rint('len of decoded str: ', len(worker.tokenizer(decoded_str, add_special_tokens=False).input_ids), 'len of control cand: ',  len(control_cand[i]))
            # print('len curr control str:', len(worker.tokenizer(curr_control, add_special_tokens=False).input_ids))
            if filter_cand:
                ...
                # elif decoded_str != curr_control and "\n" in decoded_str and len(worker.tokenizer(decoded_str, add_special_tokens=False).input_ids) == len(control_cand[i]):
                    # # decode_str = decoded_str.replace("\n", " ").replace("\t", " ") # only for llama3-8b    
                    # cands.append(decoded_str)
                else:
                    count += 1
            else:
    ...
    def run(self, 
        ...
        for j in range(1):
            ...
            model_tests = self.test(self.workers, self.prompts, find_target=True)
            _, _, _, gen_str = model_tests
            self.targets = gen_str[0]
            self.target_str = gen_str[0][0]
            print('self.target  ', self.targets)
            print('self.target_str  ', self.targets[0])

        
        #record the target we found
        with open(self.logfile, 'r') as f:
            log = json.load(f)
        log['find_target'].append(self.targets[0])
        ...
        for i in range(n_steps): 
            ...
            if success_cnt < n_trial: 
                ...
                if keep_control:
                    # print('keep control len: ', len(control))
                    print('keep control str len: ', len(self.control_str))
                    self.control_str = control
                    # print('keep control: ', self.control_str)
                    # print('control to self.control_constrol', len(self.control_str), len(control))
                
                prev_loss = loss
                if loss < best_loss:
                    best_loss = loss
                    best_control = control
                    # print('assign best loss!')
                ...
                if self.logfile is not None and (i+1+anneal_from) % test_steps == 0:
                    ...
                    fingerprint_success = self.log(i+1+anneal_from, n_steps+anneal_from, self.control_str, self.targets[0], best_loss, runtime, model_tests, verbose=verbose)
                    # if fingerprint_success:
                    #     break
                    # if len(self.control_str) < 600:
                    #     print('use last control!!')
                    #     self.control_str = last_control # weird part......
                    # else:
                    #     print('use best control!!')
                    # print('after test best_control 2: ', len(self.control_str))
                    # print('last_control to self.control_str')
            ...
            if stop_on_success:
                ...
                if all(all(tests for tests in model_test) for model_test in model_tests_mb):
                   ...
                   _ = self.log(i+1+anneal_from, n_steps+anneal_from, self.control_str, self.targets[0], best_loss, runtime, model_tests, verbose=verbose)
    ...
    def watermark_test(self, workers, prompts, include_loss=False):
        ...
        model_tests_jb = [[1] if jb == 'True' else 0 for jb in model_tests[..., 0]]
        # print('model test jb: ', model_tests_jb)
        model_tests_mb = model_tests[...,1].astype(int).tolist()
        # print('model test mb: ', model_tests_mb)
        model_tests_genstr = model_tests[...,2].astype(str).tolist()
        # print('model test genstr: ', model_tests_genstr)
        model_tests_loss = []
        if include_loss:
            for j, worker in enumerate(workers):
                worker(prompts[j], "test_loss", worker.model)
    ...
    def test_all(self):
        ...
        # print('before self.manager_pm', len(self.control_str))
        # print('in test all function self.targets: ', self.targets)
        # print('test all here !', self.targets)
    ...
    def log(self, step_num, n_steps, control, target, loss, runtime, model_tests, verbose=True):
        stop_on_success = True
        fingerprint_success = False
        ...
        # tests['control_id'] = control_str_id
        ...
        while step < n_steps:
            ...
            if num_goals < len(self.goals):
                ...
                if num_workers < len(self.workers):
                    num_workers += 1
                    loss = np.infty
                elif num_workers == len(self.workers) and stop_on_successa and fingerprint_success > n_trial:
            ...
            while counter < 1:
                if controls != prev_control and len(goals) > 0:
                    ...
                    for i in range(len(all_inputs) // batch_size + 1):
                        ...
                        print('batch: ', batch[0])
                        # batch_inputs = tokenizer(batch[0], return_tensors='pt', add_special_tokens=False) #Only for llama3-8b
...
def get_workers(params, eval=False):
    ...
    for conv in raw_conv_templates:
        if conv.name == 'one_shot' and "zero_shot" in params.conversation_templates:
            ...
            conv.sep = " "
        if "Llama2-Chinese-7b-Chat" in params.model_paths[idx]:
            conv.roles = tuple(["Human", "Assistant"])
            conv.sep_style = SeparatorStyle.ADD_COLON_SINGLE
            conv.sep2 = conv.sep2.strip()
        elif "ELYZA-japanese-Llama-2-7b-instruct" in params.model_paths[idx]:
            conv.set_system_message("あなたは誠実で優秀な日本人のアシスタントです。")
            conv.sep2 = conv.sep2.strip()
        elif "meditron-7b" in params.model_paths[idx]:
            conv.set_system_message("You are a helpful, respectful, and honest assistant. Always answer as helpfully as possible while being safe. Your answers should not include any harmful, unethical, racist, sexist, toxic, dangerous, or illegal content. Please ensure that your responses are socially unbiased and positive in nature. If a question does not make any sense, or is not factually coherent, explain why instead of answering something not correct. If you don’t know the answer to a question, please don’t share false information.")
            conv.roles = tuple(["User", "Assistant"])
        elif "Llama-2-7b-ft-instruct-es" in params.model_paths[idx]:
            conv.roles = tuple(["### Instrucción", "### Respuesta"])
            conv.set_system_message("A continuación hay una instrucción que describe una tarea, junto con una entrada que proporciona más contexto. Escriba una respuesta que complete adecuadamente la solicitud.\n\n")
